...
public class PlaynextCmd extends DJCommand
{
    ...
    public void doCommand(CommandEvent event)
    {
        ...
        {
            event.replyWarning("タイトルかURLを指定してください!");
            return;
        }
    ...
    private class ResultHandler implements AudioLoadResultHandler
    {
        ...
        private void loadSingle(AudioTrack track)
        {
            ...
            {
                m.editMessage(FormatUtil.filter(event.getClient().getWarning()+" このトラック(**"+track.getInfo().title+"**) は、 設定最大値より大きくなっています!: `"
                        +FormatUtil.formatTime(track.getDuration())+"` > `"+FormatUtil.formatTime(bot.getConfig().getMaxSeconds()*1000)+"`")).queue();
                return;
            }
            AudioHandler handler = (AudioHandler)event.getGuild().getAudioManager().getSendingHandler();
            int pos = handler.addTrackToFront(new QueuedTrack(track, event.getAuthor()))+1;
            String addMsg = FormatUtil.filter(event.getClient().getSuccess()+" 追加しました: **"+track.getInfo().title
                    +"** (`"+FormatUtil.formatTime(track.getDuration())+"`) "+(pos==0?"は、再生を始めます。":"は、"+pos+"番目に再生されます。"));
        ...
        {
            if(ytsearch)
                m.editMessage(FormatUtil.filter(event.getClient().getWarning()+"`"+event.getArgs()+"`は見つかりませんでした。")).queue();
            else
        ...
        public void loadFailed(FriendlyException throwable)
        {
            if(throwable.severity==FriendlyException.Severity.COMMON)
                ...
                m.editMessage(event.getClient().getError()+" エラーにより、この曲は指定できません。").queue();
        }
    }
}
