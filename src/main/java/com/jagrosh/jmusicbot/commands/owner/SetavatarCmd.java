...
public class SetavatarCmd extends OwnerCommand 
{
    ...
    protected void execute(CommandEvent event) 
    {
        ...
        if(s==null)
        {
            event.reply(event.getClient().getError()+" URLに問題があります。");
        }
        else
        {
            try {
            event.getSelfUser().getManager().setAvatar(Icon.from(s)).queue(
                    ...
                    t -> event.reply(event.getClient().getError()+" アバターの変更に失敗しました。"));
            } catch(IOException e) {
                event.reply(event.getClient().getError()+" URLから画像を読み込めませんでした。");
            }
        }
    }
}
